# Versioning — ask-opentelemetry

This file is this repository's **canonical versioning policy**. When
anything else in the repo (README, RELEASE.md, RELEASING.md, commit
...
Patch numbers advance by exactly one per release: `0.1.3` → `0.1.4` →
`0.1.5`. Never skip or jump patch numbers (`0.1.3` → `0.1.5` is wrong),
...
This is an `ask-*` gem: **every** `ask-*` release — and every `yamine`
release — runs through **gemchain** from the workspace root. Never
`rake release`, never a hand-run `gem build` / `gem push`:

```bash
cd /Users/kaka/Code/ask-rb
gemchain guard ask-opentelemetry
gemchain update ask-opentelemetry <new-version> --dry-run
gemchain update ask-opentelemetry <new-version>
```
...
2. Tests pass — `bundle exec rake test`.
3. Build passes — `bundle exec rake build` (or `gem build ask-opentelemetry.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/open_telemetry/version.rb` by gemchain, not by hand.
...
| `lib/ask/open_telemetry/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.1.3` → `0.1.4`.
- Several bug fixes ship together: one release, `0.1.3` → `0.1.4` (never
  `0.1.5`).
- New backwards-compatible API: `0.1.3` → `0.2.0`.
- Breaking change while pre-1.0: `0.2.0` → `0.3.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.1.4` →
  `0.1.5`, via the gemchain cascade — never hand-published.
